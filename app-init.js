...
$("viewProgramFarm").addEventListener("click",()=>{programViewMode="farm";$("viewProgramFarm").className="primary";$("viewProgramTime").className="secondary";renderProgram(plansForDate($("planViewDate").value||today()))});$("duration").addEventListener("input",finishText);$("date").addEventListener("change",finishText);$("nightDate").addEventListener("change",()=>{if(!editingPlanId){timeManuallyAdjusted=false;applySuggestedStart();updateDefaultProgramName()}});document.querySelectorAll(".compactTimePicker button[data-min]").forEach(b=>b.addEventListener("click",()=>shiftStart(Number(b.dataset.min))));$("useSuggested").addEventListener("click",()=>{timeManuallyAdjusted=false;applySuggestedStart()});$("addProduct").addEventListener("click",addProduct);$("newProductMethod").addEventListener("change",()=>{if($("newProductMethod").value==="direct")$("newProductUnit").value="L/ha";else $("newProductShuttle").value=""});$("newProduct").addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault();addProduct()}});$("addPumpRule").addEventListener("click",addPumpRule);
